...
//
// Description:
// Premium JR CALL active-call presentation screen.
//
// Visual direction:
// - Premium light JR CALL UI
// - Modern glassmorphism
// - Soft blue/cyan/green neon glow
// - Responsive mobile/tablet layout
//
// Architecture ownership:
// - Call lifecycle        -> CallService
// - Call state            -> CallProvider / CallScreenProvider
// - Audio                 -> AudioProvider / AudioManager
// - Video                 -> VideoProvider / VideoManager
...
//
// Production rules:
// - Existing public CallScreen API preserved
...
class _CallScreenState extends State<CallScreen> {
  ...
  bool get _showCallDuration {
    return widget.connectionState == CallConnectionState.connected ||
        widget.connectionState == CallConnectionState.reconnecting;
  }

  bool get _isCallTerminal {
    return widget.connectionState == CallConnectionState.ended ||
        widget.connectionState == CallConnectionState.failed;
  }
  ...
  Future<void> _runAction({
    ...
    if (callback == null || busy || _endingCall) {
      return;
    }
  ...
  Future<void> _handleEndCall() async {
    ...
    try {
      await Future<void>.sync(widget.onEndCall);
    } catch (error, stackTrace) {
  ...
  Future<void> _onKeyTap(String digit) async {
    ...
    try {
      ...
      _reportError('Keypad digit', error, stackTrace);
    }
  }

  void _removeLastDigit() {
    if (_dialedDigits.isEmpty) {
      return;
    }
                    ...
                    children: <Widget>[
                      ...
                      Expanded(
                        child: SingleChildScrollView(
                          ...
                          child: Column(
                            children: <Widget>[
                              _buildCallIdentityCard(compact: compact),
                              SizedBox(height: compact ? 16 : 22),
                              AnimatedSwitcher(
                                duration: const Duration(milliseconds: 220),
                                child: _showKeypad
                                    ? _buildKeypad(compact: compact)
                                    : _buildControlsCard(compact: compact),
                              ),
  ...
  Widget _buildHeader({required bool compact}) {
    return Container(
      ...
      decoration: const BoxDecoration(
        color: _surface,
        border: Border(bottom: BorderSide(color: Color(0xFFEAF0F8))),
      ),
      child: Row(
        children: <Widget>[
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: const LinearGradient(
                ...
                colors: <Color>[Color(0xFF03164A), _deepBlue, _cyan],
              ),
              boxShadow: const <BoxShadow>[
                BoxShadow(
                  color: Color(0x33087AF5),
                  blurRadius: 14,
                  spreadRadius: 1,
                ),
              ],
  ...
  Widget _buildCallIdentityCard({required bool compact}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const <BoxShadow>[
          ...
          BoxShadow(color: Color(0x0F04BDF5), blurRadius: 45, spreadRadius: 3),
        ],
      ),
      child: Column(
        children: <Widget>[
          ...
          AnimatedSwitcher(
            ...
            child: _showKeypad && _dialedDigits.isNotEmpty
                ? Text(
                    _dialedDigits,
                    key: ValueKey<String>(_dialedDigits),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    textAlign: TextAlign.center,
  ...
  Widget _buildSecurityPill() {
    final Color color = widget.isEncrypted ? _callGreen : _danger;
    ...
    return Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          Icon(
            widget.isEncrypted ? Icons.lock_rounded : Icons.lock_open_rounded,
            size: 15,
            color: color,
          ),
  ...
  Widget _buildCallTypeInformation() {
    return Row(
      ...
      children: <Widget>[
        Icon(
          widget.isVideoCall ? Icons.videocam_outlined : Icons.call_outlined,
          color: _textSecondary,
          size: 18,
        ),
        const SizedBox(width: 7),
        Text(
          widget.isVideoCall ? 'JR CALL Video' : 'JR CALL Voice',
          style: const TextStyle(
            color: _textSecondary,
            fontSize: 13,
  ...
  Widget _buildDuration() {
    if (widget.durationStream == null) {
      return _DurationSurface(
        child: CallTimerWidget(
          duration: Duration(
            seconds: widget.durationSeconds.clamp(0, 86400000),
          ),
        ),
      );
    }
    ...
    return StreamBuilder<int>(
      ...
      initialData: widget.durationSeconds,
      builder: (BuildContext context, AsyncSnapshot<int> snapshot) {
        final int seconds = (snapshot.data ?? widget.durationSeconds).clamp(
          0,
          86400000,
        );

        return _DurationSurface(
          child: CallTimerWidget(duration: Duration(seconds: seconds)),
        );
      },
    );
  }
  ...
  Widget _buildControlsCard({required bool compact}) {
    return Container(
      key: const ValueKey<String>('call-controls'),
      width: double.infinity,
      ...
      child: Wrap(
        ...
        children: <Widget>[
          ...
          _PremiumCallActionButton(
            tooltip: widget.isSpeakerEnabled
                ? 'Disable speaker'
                : 'Enable speaker',
            label: 'Speaker',
            icon: widget.isSpeakerEnabled
                ? Icons.volume_up_rounded
                : Icons.volume_down_rounded,
            selected: widget.isSpeakerEnabled,
            busy: _speakerBusy,
            accentColor: _callGreen,
            onPressed: widget.onToggleSpeaker == null
                ? null
                : () {
                    unawaited(
                      _runAction(
                        callback: widget.onToggleSpeaker,
                        busy: _speakerBusy,
                        setBusy: (bool value) {
                          _speakerBusy = value;
                        },
                        source: 'Toggle speaker',
                      ),
                    );
                  },
          ),
          _PremiumCallActionButton(
            tooltip: widget.isBluetoothEnabled
                ? 'Disconnect Bluetooth'
                : 'Use Bluetooth',
            label: 'Bluetooth',
            icon: Icons.bluetooth_audio_rounded,
            selected: widget.isBluetoothEnabled,
            busy: _bluetoothBusy,
            accentColor: _cyan,
            onPressed: widget.onToggleBluetooth == null
                ? null
                : () {
                    unawaited(
                      _runAction(
                        callback: widget.onToggleBluetooth,
                        busy: _bluetoothBusy,
                        setBusy: (bool value) {
                          _bluetoothBusy = value;
                        },
          ...
          _PremiumCallActionButton(
            ...
            accentColor: _deepBlue,
            onPressed: _toggleKeypad,
          ),
          if (widget.isVideoCall || widget.onToggleVideo != null)
            _PremiumCallActionButton(
              tooltip: widget.isVideoEnabled
                  ? 'Turn video off'
                  : 'Turn video on',
              label: 'Video',
              ...
              selected: widget.isVideoEnabled,
              busy: _videoBusy,
              accentColor: _primaryBlue,
              onPressed: widget.onToggleVideo == null
                  ? null
                  : () {
                      unawaited(
                        _runAction(
                          callback: widget.onToggleVideo,
                          busy: _videoBusy,
                          setBusy: (bool value) {
                            _videoBusy = value;
                          },
          ...
          if (widget.isVideoCall && widget.onSwitchCamera != null)
            _PremiumCallActionButton(
              tooltip: 'Switch camera',
              label: widget.isFrontCamera ? 'Front' : 'Back',
              ...
              onPressed: () {
                unawaited(
                  _runAction(
                    callback: widget.onSwitchCamera,
                    busy: _cameraBusy,
                    setBusy: (bool value) {
                      _cameraBusy = value;
                    },
  ...
  Widget _buildKeypad({required bool compact}) {
    return Container(
      key: const ValueKey<String>('call-keypad'),
      width: double.infinity,
      padding: EdgeInsets.fromLTRB(
        14,
        compact ? 16 : 20,
        ...
        children: <Widget>[
          Row(
            children: <Widget>[
              ...
              IconButton(
                tooltip: 'Clear keypad',
                onPressed: _dialedDigits.isEmpty ? null : _clearDigits,
          ...
          _buildKeypadRow(const <String>['1', '2', '3'], compact: compact),
          const SizedBox(height: 10),
          _buildKeypadRow(const <String>['4', '5', '6'], compact: compact),
          const SizedBox(height: 10),
          _buildKeypadRow(const <String>['7', '8', '9'], compact: compact),
          const SizedBox(height: 10),
          _buildKeypadRow(const <String>['*', '0', '#'], compact: compact),
        ],
      ),
    );
  }
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                gradient: const LinearGradient(
                  ...
                  colors: <Color>[Color(0xFFFF355E), _danger],
                ),
                borderRadius: BorderRadius.circular(24),
                boxShadow: const <BoxShadow>[
                  BoxShadow(
  ...
  void _reportError(String source, Object error, StackTrace stackTrace) {
    debugPrint('JR CALL [CallScreen/$source] error: $error');

    debugPrintStack(
      label: 'JR CALL [CallScreen/$source]',
...
class _HeaderStatusButton extends StatelessWidget {
  const _HeaderStatusButton({required this.icon, required this.color});
...
class _DurationSurface extends StatelessWidget {
  const _DurationSurface({required this.child});
...
class _PremiumCallActionButton extends StatelessWidget {
              ...
              Material(
                ...
                child: InkWell(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      shape: BoxShape.circle,
                      color: selected ? accentColor : const Color(0xFFF7FAFF),
                      border: Border.all(
                        color: selected ? accentColor : const Color(0xFFDCE7F5),
                      ),
                      boxShadow: <BoxShadow>[
                        BoxShadow(
                          color: accentColor.withValues(
                            alpha: selected ? 0.26 : 0.10,
                          ),
